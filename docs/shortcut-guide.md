...
- [WLOC设置位置](https://www.icloud.com/shortcuts/c970466843d947a195ab7f704d6c03ca)
- [wloc 清理恢复位置](https://www.icloud.com/shortcuts/bcb17b5b15a74c888c1aa950801e6894)

设置位置指令基于原作者版本，当前解析服务为 `https://wloc-spoofer.faiyun.workers.dev/api/parse`；恢复位置指令用于清除已保存的虚拟坐标。尚未独立复核新版指令的真机运行结果。安装后，在苹果地图选点 → 共享 → 选择设置指令；高德地图通过「分享 → 更多」调用。恢复指令用于清除保存值，不保证立即清除系统定位缓存。

### 替换旧解析服务

本仓库当前选点网页：https://wloc-spoofer.faiyun.workers.dev/ 。使用该站点时，将旧解析服务的域名替换为 `wloc-spoofer.faiyun.workers.dev`；解析接口为 `https://wloc-spoofer.faiyun.workers.dev/api/parse`，原有查询参数和输入变量必须保留。自行部署的用户应使用自己的域名。
...
解析接口：`GET https://wloc-spoofer.faiyun.workers.dev/api/parse?format=json&u=<URL编码的地图链接>`，成功返回 `lat`、`lon`、`name`。使用「获取词典值」的快捷指令必须保留 `format=json`，并对输入地图链接进行 URL 编码。站点根地址返回选点网页，不能代替解析接口。不带 `format=json` 时默认返回 `lat=...&lon=...` 纯文本，供旧快捷指令兼容使用。保存接口仍为 `https://gs-loc.apple.com/wloc-settings/save`，它由手机代理脚本拦截，不是 Worker 路由；不要将这个 Apple 地址替换为 Worker 域名。
